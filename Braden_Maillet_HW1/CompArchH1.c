...
////////////////////////////////////////////////////////////////////////
printf("the following value for %f is broken out as:\n", float_32.floating_value_in_32_bits);
printf("mantissa:    0x%-10x  or: ", float_32.f_bits.mantissa);
 for(j = 12; j <= 39; j++)
   {
     printf("%c" ,bit_string[j]);
   }
 printf("\n");
 printf("exponent:    0x%-10x  or: ", float_32.f_bits.exponent);
 for(k = 1; k <= 10; k++)
   {
     printf("%c" ,bit_string[k]);
   }
 printf("\n");    
 printf("sign:        0x%-10x  or: %c\n", float_32.f_bits.sign,bit_string[0]);
 printf("in base10:   %-10f    or: %s\n",float_32.floating_value_in_32_bits,bit_string);

 LABEL:clearbuffer(); //Yes I'm evil, but mostly just lazy
 printf("would you like to continue? (y/n)");
 scanf("%c", &YesNoX);
 YesNoX = tolower(YesNoX);
 if(YesNoX != 'y' && YesNoX != 'n'){
   goto LABEL;
 }
 }
}
